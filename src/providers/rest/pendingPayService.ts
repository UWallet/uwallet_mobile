...
export class PendingPayServiceProvider {
  ...
  ModifyPendingPay(id, PendingPay: ListPendingPay) {
    let body = JSON.stringify(ListPendingPay);
    let headers = new Headers({ 'Content-Type': 'application/json', 'Authorization': sessionStorage.getItem("token")});
    let options = new RequestOptions({ headers: headers });
    return this.http.put((this.apiUrl+'/lists?id='+id),body, options)
    .map((res: Response) => {
      if (res.status===201) {
        this.extractData;
      }
  ...
  DeletePendingPay(id) {
        let headers = new Headers({ 'Content-Type': 'application/json', 'Authorization': sessionStorage.getItem("token")});
        let options = new RequestOptions({ headers: headers });
        return this.http.delete((this.apiUrl+'/lists/'+id), options)
        .map((res: Response) => {
          if (res.status===201) {
            this.extractData;
          }
